...
def build_tree(X, y, feature_types):
    ...
    best_feature, best_threshold = best_split(X, y, feature_types, num_thresholds=10)
...
def train_single_tree(X_train, y_train, subsample_size=0.7):
    n_samples = int(subsample_size * X_train.shape[0])
    indices = np.random.choice(X_train.shape[0], n_samples, replace=True)
    X_sample = X_train[indices]
    y_sample = y_train[indices]

    # Build a fully expanded decision tree
    feature_types = determine_feature_types(X_sample)
    tree = build_tree(X_sample, y_sample, feature_types)

    return tree

def bagged_trees(X_train, y_train, X_test, y_test, max_trees=500, subsample_size=0.7):
    train_errors = []
    test_errors = []
    ...
    for t in range(max_trees):
        tree = train_single_tree(X_train, y_train, subsample_size)
        ...
        if (t + 1) % 50 == 0 or t == max_trees - 1:
            print(f"Trained {t + 1}/{max_trees} trees...")

    plt.figure(figsize=(10, 5))
    plt.plot(range(1, max_trees + 1), train_errors, label='Training Error')
    plt.plot(range(1, max_trees + 1), test_errors, label='Test Error')
    plt.xlabel('Number of Trees')
    plt.ylabel('Error Rate')
    plt.title('Bagged Trees Error vs Number of Trees')
    plt.legend()
    plt.show()
...
def main():
    ...
    bagged_trees(X_train, y_train, X_test, y_test, max_trees=500, subsample_size=0.7)
