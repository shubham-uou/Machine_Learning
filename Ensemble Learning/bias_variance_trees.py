...
def build_tree(X, y, feature_types):
    ...
    left_node = build_tree(X[left_indices], y[left_indices], feature_types)
    right_node = build_tree(X[right_indices], y[right_indices], feature_types)
...
def bagged_trees(X_train, y_train, X_test, y_test, num_iterations=10, num_trees=20, sample_size=300):
    single_tree_preds = np.zeros((num_iterations, X_test.shape[0]))
    bagged_preds_all_iterations = np.zeros((num_iterations, X_test.shape[0]))
    ...
    for i in range(num_iterations):
        ...
        single_tree = build_tree(X_sample, y_sample, feature_types)
        ...
        for t in range(num_trees):
            ...
            tree = build_tree(X_sample_bag, y_sample_bag, feature_types)
            ensemble_preds[t] = predict(X_test, tree)

        bagged_preds_all_iterations[i] = np.mean(ensemble_preds, axis=0)

    single_tree_bias, single_tree_variance = compute_bias_variance(single_tree_preds, y_test)

    bagged_tree_bias, bagged_tree_variance = compute_bias_variance(bagged_preds_all_iterations, y_test)
...
def main():
    ...
    single_tree_bias, single_tree_variance, bagged_tree_bias, bagged_tree_variance = bagged_trees(X_train, y_train, X_test, y_test)
    ...
    print(f"Bagged Trees Bias^2: {bagged_tree_bias}, Variance: {bagged_tree_variance}, General Squared Error: {bagged_tree_bias + bagged_tree_variance}")
