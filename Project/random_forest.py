...
Original file is located at
    https://colab.research.google.com/drive/15QTxmN_9VkBIaV0Jc_WySc0UNsxSzY2E
"""
...
def train_single_tree(X_train, y_train, feature_types, max_depth=10):
    n_samples = X_train.shape[0]
    indices = random.choices(range(n_samples), k=n_samples)
    X_sample = X_train[indices]
    y_sample = y_train[indices]
    tree = build_tree(X_sample, y_sample, feature_types, max_depth=max_depth)
    return tree

def random_forest(X_train, y_train, X_test, num_trees=10, max_depth=10):
    feature_types = determine_feature_types(X_train)
    trees = [train_single_tree(X_train, y_train, feature_types, max_depth) for _ in range(num_trees)]

    predictions = np.zeros((len(X_test), num_trees))
    for i, tree in enumerate(trees):
        predictions[:, i] = predict_proba(X_test, tree)
...
def main():
    ...
    predictions = random_forest(X_train, y_train, X_test, num_trees=10, max_depth=10)
